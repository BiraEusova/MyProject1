...
if (isset($_POST["MAX_FILE_SIZE"])) {
    ...
    if (count($err) == 0) {
        ...
        move_uploaded_file($_FILES["filename"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . "/MyProject1/files/" . $_FILES["filename"]["name"]);
    } else {
...
if (isset($_POST["file_delete"])) {
    $file_name = $_SERVER['DOCUMENT_ROOT'] . "/MyProject1/files/" . $_POST["file_delete"];
    unlink($file_name);
}
...
<?php
$image_dir_path = $_SERVER['DOCUMENT_ROOT']."/MyProject1/files";
...
if ($array_files_count)
{
    ...
    {
        ?>
        <p><a href="/files/<?php echo $array_files[$i];
            ?>" target="_blank">
